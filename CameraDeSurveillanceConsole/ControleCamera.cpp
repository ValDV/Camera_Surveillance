...
bool ControleCamera::checkPort()
{
    ...
    std::cout << "Erreur: Port série non ouvert." << std::endl;
    return false;
}
...
bool ControleCamera::writeToPort(const QString& command)
{
    ...
    if (bytesWritten == -1)
    {
        std::cout << "Erreur lors de l'envoi de la commande: " << port->errorString().toStdString() << std::endl;
        return false;
    }
...
void ControleCamera::onSerialPortReadyRead()
{
    ...
    std::cout << "Données reçues: " << receivedData.toStdString() << std::endl;
    ...
    if (receivedData.contains('Q') || receivedData.contains('R'))
    {
        ...
        std::cout << "Réponse reçue, confirmation!" << std::endl;
    }
}